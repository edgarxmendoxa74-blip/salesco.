...
export const LiveCopilot: React.FC<LiveCopilotProps> = ({ onEndCall }) => {
  ...
  return (
    <div className="space-y-3 sm:space-y-4 max-w-5xl mx-auto pb-16 px-1 sm:px-0">
      ...
      {/* TOP COPILOT HEADER - High Contrast */}
      <div className="bg-white p-3.5 sm:p-4 rounded-2xl border border-slate-300 shadow-md space-y-3 sm:space-y-0 sm:flex sm:items-center sm:justify-between">
        <div className="flex items-center justify-between sm:justify-start gap-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h1 className="font-black text-base sm:text-lg text-slate-900 tracking-tight leading-none">SalesCo</h1>
              <div className="flex items-center gap-1 mt-1 text-xs">
                ...
                <input type="text" value={prospectName} onChange={(e) => setProspectName(e.target.value)}
                  className="bg-slate-100 border border-slate-300 rounded px-2 py-0.5 text-xs text-blue-900 font-bold focus:outline-none focus:border-blue-600 max-w-[130px] sm:max-w-none" />
              </div>
            </div>
          </div>
          <div className="px-2.5 py-1 rounded-lg bg-slate-100 border border-slate-300 text-xs font-mono font-bold text-slate-900 sm:hidden">
        ...
        <div className="flex items-center justify-between sm:justify-end gap-2 pt-2 sm:pt-0 border-t border-slate-200 sm:border-0">
          <div className="flex items-center gap-2">
            {isRecording ? (
              ...
              <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-slate-100 text-slate-600 text-xs font-bold border border-slate-300">
                <MicOff className="w-3.5 h-3.5" /><span>PAUSED</span>
              </div>
            )}
            <div className="hidden sm:flex px-3 py-1.5 rounded-xl bg-slate-100 border border-slate-300 text-xs font-mono font-bold text-slate-900 items-center gap-1.5">
              <span className="text-slate-500 text-[10px]">TIME</span>
              <span className="text-blue-700">{formatDuration(seconds)}</span>
            </div>
          </div>

          {!isSessionActive ? (
            <button onClick={handleStartSession}
              className="px-4 py-2 rounded-xl bg-blue-700 hover:bg-blue-800 text-white font-extrabold text-xs shadow-lg flex items-center gap-1.5 transition-all">
              <Mic className="w-4 h-4 text-white" /><span>Start Call</span>
            </button>
          ) : (
            <button onClick={handleFinishCall}
              className="px-4 py-2 rounded-xl bg-red-600 hover:bg-red-700 text-white font-extrabold text-xs shadow-lg flex items-center gap-1.5 transition-all">
              <PhoneOff className="w-4 h-4 text-white" /><span>End Call</span>
            </button>
          )}
        </div>
      </div>
      ...
      {/* SALES STAGE BAR */}
      <div className="bg-white p-3 sm:p-4 rounded-2xl border border-slate-300 shadow-sm overflow-x-auto no-scrollbar">
        <div className="flex items-center justify-between text-xs mb-2 min-w-[300px]">
          ...
          <span className="font-extrabold text-blue-700 flex items-center gap-1 text-xs bg-blue-50 px-2 py-0.5 rounded border border-blue-200">
            <Zap className="w-3.5 h-3.5 text-blue-600" />{aiAnalysis.current_stage.toUpperCase().replace('_', ' ')}
          </span>
        </div>
            ...
            return (
              <div key={s.key}
                className={`py-2 px-2 sm:py-2.5 sm:px-1 text-center rounded-xl transition-all duration-200 shrink-0 sm:shrink ${
                  isActive
                    ? 'bg-blue-700 text-white font-black ring-2 ring-blue-500 shadow-md'
                    : isPast
      ...
      <div className="flex lg:hidden rounded-xl bg-slate-200 p-1 font-bold text-xs border border-slate-300">
        <button
          ...
          className={`flex-1 py-2 rounded-lg transition-all flex items-center justify-center gap-1.5 ${
            activeTab === 'copilot' ? 'bg-white text-blue-800 shadow font-black' : 'text-slate-700'
          }`}
        >
        ...
        <button
          ...
          className={`flex-1 py-2 rounded-lg transition-all flex items-center justify-center gap-1.5 ${
            activeTab === 'transcript' ? 'bg-white text-blue-800 shadow font-black' : 'text-slate-700'
          }`}
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
        ...
        <div className={`lg:col-span-2 space-y-4 ${activeTab === 'transcript' ? 'hidden lg:block' : 'block'}`}>
          ...
          <div className="bg-white p-4 rounded-2xl border border-slate-300 shadow-sm">
            ...
            <p className="text-base sm:text-lg font-bold text-slate-900 italic leading-snug">"{latestProspectMsg || 'Listening to prospect speech...'}"</p>
          </div>

          {/* AI SUGGESTED RESPONSE CARD - ULTRA HIGH CONTRAST */}
          <div className="bg-white p-5 sm:p-6 rounded-2xl border-2 border-blue-600 shadow-xl relative overflow-hidden">
            <div className="flex items-center justify-between mb-3">
              <span className="px-3 py-1 rounded-md bg-blue-100 text-blue-900 text-xs font-black border border-blue-300 flex items-center gap-1.5">
                <Sparkles className="w-4 h-4 text-blue-700" /> AI SUGGESTED RESPONSE
              </span>
              <button onClick={handleCopyResponse}
                className="flex items-center gap-1 px-3 py-1 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-bold border border-slate-300">
                {copied ? <Check className="w-4 h-4 text-emerald-600" /> : <Copy className="w-4 h-4 text-slate-700" />}
                <span>{copied ? 'Copied!' : 'Copy'}</span>
              </button>
            </div>

            <div className="p-4 sm:p-5 rounded-xl bg-blue-50 border border-blue-300 shadow-inner mb-4">
              <p className="text-lg sm:text-2xl font-extrabold text-slate-950 leading-relaxed tracking-tight">"{aiAnalysis.suggested_response}"</p>
            </div>

            {aiAnalysis.next_question && (
              <div className="mb-4 p-3 rounded-xl bg-indigo-50 border border-indigo-300 flex items-start gap-2.5">
                <HelpCircle className="w-5 h-5 text-indigo-700 mt-0.5 shrink-0" />
                <div className="text-xs sm:text-sm">
                  <span className="font-extrabold text-indigo-950">Recommended Follow-up Question:</span>
            ...
            {/* QUICK ACTIONS */}
            <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pt-3 border-t border-slate-200">
              {([
                { action: 'SHORTER' as const, icon: Scissors, label: 'Shorter', color: 'text-blue-700' },
                { action: 'MORE_NATURAL' as const, icon: Sparkles, label: 'Natural', color: 'text-emerald-700' },
                { action: 'MORE_PERSUASIVE' as const, icon: Flame, label: 'Persuasive', color: 'text-orange-700' },
                { action: 'NEXT_QUESTION' as const, icon: MessageSquarePlus, label: 'Next Question', color: 'text-purple-700' },
                { action: 'HANDLE_OBJECTION' as const, icon: ShieldCheck, label: 'Objection', color: 'text-red-700' },
              ]).map(({ action, icon: Icon, label, color }) => (
                <button key={action} onClick={() => handleQuickAction(action)}
                  className="px-3 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-900 text-xs font-bold flex items-center gap-1.5 border border-slate-300 shadow-sm shrink-0 whitespace-nowrap">
          ...
          {/* AI ANALYSIS CARD */}
          <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-300 shadow-sm space-y-3">
            <h3 className="text-xs font-black text-slate-600 uppercase tracking-wider flex items-center gap-1.5">
              <Zap className="w-4 h-4 text-amber-600" /> REAL-TIME AI ANALYSIS
            </h3>
            <div className="grid grid-cols-3 gap-2 sm:gap-3">
              <div className="p-2.5 sm:p-3 rounded-xl bg-slate-100 border border-slate-300">
                <span className="text-[10px] text-slate-600 uppercase block font-extrabold">Detected</span>
                <span className="text-xs sm:text-sm font-black text-blue-800 truncate block mt-0.5">{aiAnalysis.detected_label || 'EXISTING SOLUTION'}</span>
              </div>
              <div className="p-2.5 sm:p-3 rounded-xl bg-slate-100 border border-slate-300">
                <span className="text-[10px] text-slate-600 uppercase block font-extrabold">Pain Point</span>
                <span className="text-xs sm:text-sm font-black text-amber-800 truncate block mt-0.5">{aiAnalysis.pain_points[0]?.name || 'STATIC MENU'}</span>
              </div>
              <div className="p-2.5 sm:p-3 rounded-xl bg-slate-100 border border-slate-300">
                <span className="text-[10px] text-slate-600 uppercase block font-extrabold">Buying Intent</span>
                <span className={`text-xs sm:text-sm font-black uppercase truncate block mt-0.5 ${
                  aiAnalysis.intent === 'high' ? 'text-emerald-800' : aiAnalysis.intent === 'medium' ? 'text-blue-800' : 'text-slate-700'
                }`}>{aiAnalysis.intent}</span>
              </div>
            </div>
            <p className="text-xs text-slate-800 leading-relaxed bg-slate-100 p-3 rounded-xl border border-slate-300 font-semibold">
              <span className="font-black text-slate-950">AI Logic: </span>{aiAnalysis.reason}
            </p>
          </div>
        </div>
        ...
        <div className={`space-y-4 flex flex-col ${activeTab === 'copilot' ? 'hidden lg:flex' : 'flex'}`}>
          {/* DEMO CONTROLS */}
          <div className="bg-white p-4 rounded-2xl border-2 border-indigo-300 shadow-sm bg-indigo-50/50">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-black text-indigo-900 flex items-center gap-1.5">
                <Play className="w-4 h-4 text-emerald-600 fill-emerald-600" /> DEMO SIMULATION
              </span>
            </div>
            <button onClick={handleNextDemoStep}
              className="w-full py-2.5 px-3 rounded-xl bg-indigo-700 hover:bg-indigo-800 text-white font-extrabold text-xs shadow-md flex items-center justify-center gap-1.5 mb-2 transition-all">
              <ChevronRight className="w-4 h-4 text-white" /><span>Simulate Next Prospect Voice</span>
            </button>
            <div className="flex items-center gap-2 mt-2 pt-2 border-t border-slate-300">
              <input type="text" value={customInputText} onChange={(e) => setCustomInputText(e.target.value)}
                ...
                className="flex-1 bg-white border border-slate-300 rounded-lg px-3 py-1.5 text-xs text-slate-900 font-semibold placeholder-slate-500 focus:outline-none focus:border-blue-600" />
              <button onClick={() => handleSendCustomText('prospect')} className="p-1.5 rounded-lg bg-blue-700 text-white font-extrabold text-xs px-3">
                Send
              </button>
            </div>
          </div>
          ...
          {/* LIVE TRANSCRIPT */}
          <div className="bg-white p-4 rounded-2xl border border-slate-300 shadow-sm flex-1 flex flex-col justify-between min-h-[340px]">
            <div className="flex items-center justify-between mb-3 border-b border-slate-200 pb-2">
              <span className="text-xs font-black text-slate-900 uppercase tracking-wider flex items-center gap-1.5">
                <Volume2 className="w-4 h-4 text-blue-700" /> LIVE TRANSCRIPT
              </span>
              <span className="text-[10px] text-emerald-800 font-mono font-black bg-emerald-100 px-2 py-0.5 rounded border border-emerald-300">REALTIME</span>
            </div>
            <div className="space-y-3 overflow-y-auto max-h-[360px] pr-1 flex-1">
              {transcripts.map((t) => (
                <div key={t.id} className={`p-3 rounded-xl text-xs space-y-1 ${
                  t.speaker === 'prospect'
                    ? 'bg-slate-100 border border-slate-300 text-slate-900 mr-2'
                    : 'bg-blue-100 border border-blue-300 text-blue-950 ml-2'
                }`}>
                  <div className="flex items-center justify-between text-[10px]">
                    <span className={`font-black uppercase ${t.speaker === 'prospect' ? 'text-blue-800' : 'text-emerald-800'}`}>
